...
function initializeDropZone() {
    ...
    dropZone.addEventListener('dragover', (e) => {
        e.preventDefault();
        dropZone.classList.add('border-[#f89c00]', 'bg-[#2A2A3E]');
        dropZone.classList.remove('border-[#3C3F58]', 'bg-[#1F2235]');
    });

    dropZone.addEventListener('dragleave', (e) => {
        e.preventDefault();
        dropZone.classList.remove('border-[#f89c00]', 'bg-[#2A2A3E]');
        dropZone.classList.add('border-[#3C3F58]', 'bg-[#1F2235]');
    });
...
function displayPaymentRequest(paymentData) {
        ...
        <div class="space-y-6">
            ...
            <div class="text-center">
                <div class="inline-flex items-center gap-2 text-sm text-orange-600 bg-orange-50 px-3 py-2 rounded-lg">
                    <span>⏰</span>
                    Payment expires in <span id="paymentTimer">${expires_in_minutes || 15}</span> minutes
                </div>
            </div>
            ...
            <div class="text-center mt-4">
                <button onclick="skipPaymentForDemo()" 
    ...
    startPaymentTimer(expires_in_minutes || 15);
}
    ...
    paymentCheckInterval = setInterval(async () => {
        try {
            ...
            if (result.success && result.payment_status) {
                ...
                console.log('Payment status:', status, result.payment_status);
            }
        } catch (error) {
...
// This duplicate function was removed - using the one defined earlier in the file
...
// Demo function to skip payment for testing
function skipPaymentForDemo() {
    ...
    const mockPaymentResult = {
        ...
        explorer_url: 'https://polygonscan.com/tx/0x' + Array.from({length: 64}, () => Math.floor(Math.random() * 16).toString(16)).join('')
    };
...
window.skipPaymentForDemo = skipPaymentForDemo;
window.closePermanentStorageModal = closePermanentStorageModal;

// Debug log to confirm module loaded
console.log('✅ Permanent Storage module loaded successfully');

// Note: Initialization moved to dashboard.js to avoid conflicts
// This module is now legacy - functionality moved to arweave-payment.js
