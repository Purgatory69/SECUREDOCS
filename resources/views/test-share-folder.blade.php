<!DOCTYPE html>
<!-- 
    This file is a surgical merge of:
    1. Your ORIGINAL `share-download.txt` (from 11-13)
    2. Your NEW `share-download-file test.txt` (from 11-14)
-->
<html lang="en">
<head>
    <meta charset="UTF-8">
    ...
    <!-- This title is from your original file -->
    ...
    <!-- 
        This <style> block is from your ORIGINAL file. 
        It is needed for the FOLDER VIEW styles (like breadcrumbs).
    -->
    <style>
        body {
            font-family: 'Inter', sans-serif;
            /* The main background color is now set on the <body> tag below,
               using your app's theme color #1D1D2F.
            */
        }

        /* Styles for FOLDER VIEW from your original file */
        .folder-table {
            background: #1f2937;
            border-radius: 8px;
            overflow: hidden;
...
<!-- 
    The new dark theme background is applied, as seen in your test file.
-->
<body style="background-color: #1D1D2F;" class="text-white">
    
    <!-- 
        ======================================================================
        THIS IS THE MAIN if STATEMENT FROM YOUR ORIGINAL FILE
        ======================================================================
    -->
    @if($file->is_folder)
        <!-- 
            ======================================================================
            FOLDER VIEW (UNTOUCHED)
            This is the entire FOLDER VIEW from your original file.
            It is exactly as you had it.
            ======================================================================
        -->
        <div class="min-h-screen bg-gray-900">
            ...
            <!-- OTP Files Warning (from original file) -->
            ...
            <!-- Main Content (from original file) -->
            <div class="max-w-7xl mx-auto p-4">
                ...
                <!-- Download Actions (from original file) -->
    ...
    <!-- 
        ======================================================================
        THIS IS THE @else REPLACEMENT
        
        This block replaces the OLD file view with the NEW file view
        from your test file, but with all the ORIGINAL blade variables
        and routes re-inserted.
        ======================================================================
    -->
    @else
        <!-- File View - NEW DARK DESIGN -->
        <!-- This wrapper makes the header, content, and footer stick correctly -->
        <div class="flex flex-col min-h-screen"> 
            
            <!-- NEW Header (from your test file) -->
            <div class="bg-[#141326] px-6 py-6">
                <div class="flex items-center justify-between w-full">
                    ...
                    <!-- 
                        Login/Signup Buttons (MERGED)
                        - Using your new design, but with the correct `route()` helpers
                          from your original Folder View for robustness.
                    -->
            ...
            <!-- NEW Back Button Script (from your test file) -->
            <!-- This script is placed here so it has access to the button above -->
            <script>
            document.addEventListener('DOMContentLoaded', function() {
                const backButton = document.getElementById('back-button');

                if (backButton) { // Check if button exists
            ...
            <!-- NEW Language Toggle (from your test file) -->
            <div class="fixed bottom-6 right-6 z-50">
                <div class="relative">
                    ...
                    <!-- 
                        Dropdown Menu (MERGED)
                        - Using your test file's UI
                        - Using the `route()` helpers from your test file
                    -->
            ...
            <!-- NEW Language Toggle Script (from your test file) -->
            <script>
                document.addEventListener('DOMContentLoaded', function() {
            ...
            <!-- 
                NEW MAIN CONTENT AREA (from your test file)
                - This `flex-1` class is what makes the card center vertically.
            -->
            <div class="container mx-auto px-6 py-8 flex-1 flex items-center justify-center">

                <!-- 
                    NEW SHARE CARD (from your test file)
                -->
                <div class="bg-[#3C3F58] w-full max-w-lg p-8 mb-4 md:p-12 rounded-2xl">
                    
                    <!-- File Icon (from your test file) -->
                    <div class="flex justify-center mb-6">
                        <img src="{{ asset('file.png') }}" alt="File Icon" class="w-12 h-12">
                    </div>

                    <!-- 
                        File Name (SURGICALLY MERGED)
                        - Uses new 'truncate' class from test file
                        - Uses original '$file->file_name' variable
                    -->
                    ...
                    <!-- 
                        File Details (SURGICALLY MERGED)
                        - Uses new left-aligned layout from test file
                        - Uses original backend variables
                    -->
                    <div class="space-y-2 text-sm text-gray-300 mb-10">
                        <p>
                            <span class="font-medium text-gray-100">Shared By :</span>
                            <!-- This variable is from your original FOLDER view, as your new design needs it -->
                            {{ $share->user->name }}
                        </p>
                        <p>
                            <span class="font-medium text-gray-100">Share Link Expires in :</span>
                            <!-- This logic is from your original file view's "About" section -->
                            {{ $share->expires_at ? $share->expires_at->format('M j, Y g:i A') : 'Never' }}
                        </p>
                    </div>

                    <!-- 
                        Button Layout (SURGICALLY MERGED)
                    -->
                    <div class="grid grid-cols-2 gap-4">
                        <!-- 
                            Save to My Files Button (MERGED)
                            - Uses new style from test file
                            - Uses original 'onclick="saveToMyFiles()"'
                        -->
    ...
    <!-- 
        ...
        This entire <script> block is from your ORIGINAL file.
        It contains all the critical functions like saveToMyFiles(),
        checkShareValidity(), openFile(), etc.
        ======================================================================
    -->
    <script>
        ...
        function toggleListView() {
            ...
            if (!table || !listBtn || !gridBtn) return; // Exit if not in folder view
        ...
        function toggleGridView() {
            ...
            if (!gridView) {
                ...
                if(table.parentNode) { // Ensure table has a parent before appending
            ...
            if(gridView) gridView.style.display = 'grid'; // Check if gridView was created
        }


        function createGridView() {
            const gridView = document.createElement('div');
            // Added p-4 to match your old grid style, you can remove if not needed
        ...
        async function openFile(fileId, fileName, isFolder) {
            try {
                ...
                // Fallback to old method (from original file)
                if (isFolder) {
